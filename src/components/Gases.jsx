...
//////////////////////////////////////////////
    ...
    const addGases=async(e) =>{
        e.preventDefault();
        if (nameValue !== "" && descValue !== "") { // لازم يعبي
     ...
     const deleteItem=async(id)=> {  //// ال اي هذي اللي هنا هذا جاي من الفانكشمن اللي تحت
        ...
        <div>
            ...
            {myData.metal && <div className="container py-5 ">
                ...
                <div className="row ">
                    ...
                    <div className="row">
                        {myData.metal.map((item, index) => <MetalItems token={token} getData={getData}  item={item} forminput={forminput} setValue={setValue} key={index} deleteItem={deleteItem} isAdmin={isAdmin} />)}
                        ...
                        {!add ? <button onClick={showForm} className='btn btn-secondary'>Add New Metal</button> :
                            <form style={{ border: "1px solid #444", borderRadius: "5px" }} className="text-center shadow-lg pt-3 px-3">
                                ...
                                <button onClick={addGases} className='btn btn-danger my-4 px-5'>Add</button>
                            </form>
                        }
                    </div> : ""
                    }
